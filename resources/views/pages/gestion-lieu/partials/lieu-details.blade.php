...
@if (!empty($lieu))
    <x-common.table :data="$lieu['information_details']" :title="__('livewire/tables/lieu_table.details')" />
    ...
    <button wire:click="$dispatch('lieu-edit', { lieu: @js($lieu) })"  class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none
      ...
      {{ __('livewire/tables/lieu_table.modifier')}}
    </button>
    ...
    {{__('livewire/tables/lieu_table.supprimer')}}
</button>
</div>
