<div>
    ...
    <div x-data="{ loading: @entangle('loading') }" class="container mx-auto p-4">
        <x-common.loader x-show="loading"/>
    </div>
                ...
                <div id="vertical-pills-tabcontent" class="p-6 w-full">
                    ...
                    <div id="profile-content" class="p-6 bg-gray-50 text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg w-full"
                        ...
                        <x-pages.residances.details-table :rows="
                                            [
                                                ['Code', $residence['identifiant'] ?? ''],
                                                ['Type', $residence[''] ?? ''],
                                                ['Sous Type', $residence['type_nc_full_name'] ?? ''],
                                                ['Appartenance', $residence['appartenance'] ?? ''],
                                                ['Etat', $residence['etat_strecture_full_name'] ?? ' ']
                                            ]" />
                    </div>
                    <div id="content-content" class="hidden p-6 bg-gray-50 text-medium
                        ...
                        <h3 class="text-lg font-bold   text-gray-900 dark:text-white mb-2">Plus de détails</h3>
                        <x-pages.residances.details-table :rows="
                                            [
                                                ['Code', $residence['identifiant'] ?? ''],
                                                ['Date Creation', $residence['date_creation'] ?? ''],
                                                ['Date Ouverture', $residence['date_ouverture'] ?? ''],
                                                ['Numero d\'Inscription Domaniale', $residence['numero_inscription_domaniale'] ?? ''],
                                                ['Date d\'Inscription Domaniale', $residence['date_inscription_domaniale'] ?? ''],
                                                ['Capacité Théorique', $residence['capacite_theorique'] ?? 0],
                                                ['Capacité Réel', $residence['capacite_reel'] ?? 0],
                                                ['Nombre des places disponibles (Garçons)', $residence['places_disponibles_garcons'] ?? 0],
                                                ['Nombre des places disponibles (Filles)', $residence['places_disponibles_filles'] ?? ''],
                                                ['Surface Globale', $residence['surface_globale'] ?? ''],
                                                ['Surface Batie', $residence['surface_batie'] ?? ''],
